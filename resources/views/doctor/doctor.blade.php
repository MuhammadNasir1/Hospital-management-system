                            ...
                            <div>
                                ...
                                <div class="mt-3" id="medicines">
                                    @foreach ($data['medicine'] as $item)
                                        <div class="py-2 cursor-pointer border-b medicine" data-id="{{ $item->id }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            $('.medicine').on('click', function() {
                ...
                $.ajax({
                    url: '../doctor/fetchMedicine/' + medicineId,
                    type: 'GET',
                    success: function(response) {
                        if (response.medicine) {
                            // Populate modal with the fetched medicine data
                            $('#modalMedicineName').text(response.medicine.name);
                            $('#medicineId').val(response.medicine.id);
